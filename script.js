...
function initVisualAdminModule() {
  ...
  // Sinh slug duy nhất: ưu tiên họ tên đầy đủ, thêm _2 _3 nếu trùng
  function resolveUniqueSlug(baseSlug, existingList, currentId = null) {
    const taken = (s) => existingList.some(g => g.slug === s && g.id !== currentId);
    if (!taken(baseSlug)) return { slug: baseSlug, changed: false };
    for (let i = 2; i <= 99; i++) {
      const numbered = `${baseSlug}_${i}`;
      if (!taken(numbered)) return { slug: numbered, changed: true };
    }
    return { slug: `${baseSlug}_${Date.now()}`, changed: true };
  }
  ...
  function autoSuggestGuestFields() {
    ...
    // Tên hiển thị: danh xưng + tên cuối
    ...
    // Slug cơ bản dùng họ tên đầy đủ (fallback: tên hiển thị)
    const slugBase = fullname || (guestDisplaynameInput && guestDisplaynameInput.value.trim()) || suggestedDisplay;
    const suggestedSlug = `Thiep_cuoi_gui_${slugifyVietnamese(slugBase)}`;
    ...
    // Cảnh báo real-time nếu slug gợi ý bị trùng
    const warningEl = document.getElementById('admin-guest-slug-warning');
    if (warningEl && guestSlugInput) {
      const currentSlug = guestSlugInput.value.trim();
      const currentList = getLocalGuests();
      const isDuplicate = currentList.some(g => g.slug === currentSlug);
      if (isDuplicate) {
        warningEl.textContent = '⚠️ Link này đã tồn tại — khi tạo sẽ tự động thêm số để phân biệt.';
  ...
  if (guestDisplaynameInput) {
    guestDisplaynameInput.addEventListener('input', () => {
      ...
      // Khi sửa tên hiển thị tay: slug vẫn ưu tiên họ tên đầy đủ
      const fullnameVal = guestFullnameInput ? guestFullnameInput.value.trim() : '';
      const slugBase = fullnameVal || guestDisplaynameInput.value.trim();
      if (guestSlugInput && guestSlugInput.dataset.autoGenerated !== 'false') {
        guestSlugInput.value = `Thiep_cuoi_gui_${slugifyVietnamese(slugBase)}`;
      }
      autoSuggestGuestFields(); // cập nhật cảnh báo
    });
  }
  ...
  if (guestSlugInput) {
    guestSlugInput.addEventListener('input', () => {
      ...
      // Kiểm tra trùng khi người dùng sửa slug tay
      const warningEl = document.getElementById('admin-guest-slug-warning');
      if (warningEl) {
        const currentList = getLocalGuests();
        const isDuplicate = currentList.some(g => g.slug === guestSlugInput.value.trim());
        if (isDuplicate) {
          warningEl.textContent = '⚠️ Link này đã tồn tại — khi tạo sẽ tự động thêm số để phân biệt.';
          warningEl.style.display = 'block';
        } else {
  ...
  if (guestConfirmBtn) {
    guestConfirmBtn.addEventListener('click', async () => {
      ...
      // Slug cơ bản: ưu tiên họ tên đầy đủ, fallback tên hiển thị
      const slugBaseAuto = `Thiep_cuoi_gui_${slugifyVietnamese(fullName || displayName)}`;
      const baseSlug = slugInputVal || slugBaseAuto;
      const guestOfChoice = document.querySelector('input[name="admin_guest_of_choice"]:checked')?.value || 'bride';

      // Phát hiện và xử lý trùng slug
      const list = getLocalGuests();
      const { slug: finalSlug, changed: slugChanged } = resolveUniqueSlug(baseSlug, list, null);

      // Cập nhật field slug nếu đã thay đổi
      if (slugChanged && guestSlugInput) {
        guestSlugInput.value = finalSlug;
        guestSlugInput.dataset.autoGenerated = 'false';
      }
      // Ẩn cảnh báo trùng vì đã xử lý xong
      const warningEl = document.getElementById('admin-guest-slug-warning');
      if (warningEl) warningEl.style.display = 'none';

      const fullUrl = `${window.location.origin}/${finalSlug}`;
      ...
      const existingIdx = list.findIndex(g => g.slug === finalSlug);
      if (existingIdx >= 0) {
      ...
      if (slugChanged) {
        showAdminToast(`⚠️ Slug trùng! Đã tự động đổi thành "${finalSlug}". Thiệp "${displayName}" đã được tạo!`);
      } else {
